...
Frame::Frame(const Frame &frame)
    :mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     yoloBoundingBoxList(frame.yoloBoundingBoxList),
...
{
    if(flag==0){
        (*mpORBextractorLeft).ExtractORBKeyPoints(imgray,cv::Mat(),mvKeyPoints);
    }
    else{
        (*mpORBextractorRight).ExtractORBKeyPoints(imgray,cv::Mat(),mvKeyPoints);
    }
}
...
{
    if(flag==0){
        (*mpORBextractorLeft).ProcessDesp(imgray,cv::Mat(),mvKeyPoints,mvKeys,mDescriptors);
    }
    else{
        (*mpORBextractorLeft).ProcessDesp(imgray,cv::Mat(),mvKeyPoints,mvKeysRight,mDescriptorsRight);
    }
}
...
void Frame::RemoveMovingKeyPoints(){
    if(this->yoloBoundingBoxList.empty()){
        return;
    }
    
    float scale = 0;
    int nlevel = mpORBextractorLeft->GetLevels();
    std::vector<float> scaleFactor = mpORBextractorLeft->GetScaleFactors();
    for(int level = 0; level < nlevel; level++){
        vector<cv::KeyPoint>& tempKeypoints = mvKeyPoints[level];
        ...
        //         YoloBoundingBox tempBox = yoloBoundingBoxList[i];
        //         if(tempBox.GetLabel() != "person"){continue;}
        ...
        //         {
        //             keypoint=tempKeypoints.erase(keypoint);
        //             flag = false;
        //             break;
        //         }
        //     }
        ...
        mvKeyPoints[level].swap(keyPointsWithoutMovingPoints);
    }
}
...
void Frame::BuildMapPlanePointCloud(const cv::Mat &imDepth, const cv::Mat &segmentImage, int cloudStep)
{
    ...
    for(int i = 0; i < mvpMapPlanes.size(); i++){

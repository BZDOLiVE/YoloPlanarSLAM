...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mvpMapPlanes(F.mvpMapPlanes), mbNewPlane(F.mbNewPlane), mvPlaneCameraCoefficients(F.mvPlaneCameraCoefficients), mnPlaneNum(F.mnPlaneNum)
{
    mnId=nNextId++;
...
{
    unique_lock<mutex> lock(mMutexFeatures);
    if(idx < mvpMapPlanes.size() && idx >= 0){
...
{
    int idx = pMP->GetIndexInKeyFrame(this);
    if(idx >= 0 && idx < mvpMapPlanes.size()){
        mvpMapPlanes[idx]=static_cast<MapPlane*>(NULL);
    }
    return;
}

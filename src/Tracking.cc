...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor):
    ...
    mYoloDetector(ConfigFile::ncnnCPUNum), mPlaneDetector(), mPlaneMatcher(), mPlaneEffect(3),
    framePerSecond(0)
{
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
    ...
    cv::Mat imageForYolo = imRGB;

    // double t5 = cv::getTickCount();
    mYoloDetector.YoloObjectDetect(imageForYolo, mCurrentFrame.yoloBoundingBoxList, 320, 320);
    // double t6 = cv::getTickCount();
    // double time_elapsed = (t6-t5)/(double)cv::getTickFrequency();
    // cout<<"Total yolo time elapsed: "<<time_elapsed<<endl;

    //remove dynamic keypoints
    mCurrentFrame.RemoveMovingKeyPoints();

    //calculate descriptors
    mCurrentFrame.ExtractORBDesp(0, mImGray);

    // double t7 = cv::getTickCount();
    mPlaneDetector.Process(imDepth, mCurrentFrame.mvPlaneCameraCoefficients, mCurrentFrame.mvCylinderCameraCoefficients);
    // double t8 = cv::getTickCount();
    // double time_elapsed_plane = (t8-t7)/(double)cv::getTickFrequency();
    // cout<<"Total plane time elapsed: "<<time_elapsed_plane<<endl;
    
    mCurrentFrame.MyCalculateAfterRemoveDynamicPointsAndPlaneExtraction(imDepth);
